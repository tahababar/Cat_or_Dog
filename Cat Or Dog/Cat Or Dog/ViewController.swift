...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    @IBOutlet weak var answer: UIBarButtonItem!
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let image = info[.originalImage] as? UIImage { //do .editedImage if allowsediting is true
            viewResult.image = image       //assigning that image to our image view
    ...
    func detect(image: CIImage) {
        ...
        guard let model = try? VNCoreMLModel(for: CatOrDog().model) else {
        ...
        let request = VNCoreMLRequest(model: model) { request, error in
            guard let results = request.results as? [VNClassificationObservation],
                ...
                DispatchQueue.main.async {
                 ...
                 self.answer.title = "Result: \(topResult.identifier)".capitalized; //to capitalize our string
                }
        }
